...
window.onload = function () {
  ...
  const muteMoveButton = document.getElementById("mute-move-btn");
  const backgroundSound = document.getElementById("background-sound");
  const moveASound = document.getElementById("moveA-sound");
  const moveDKSound = document.getElementById("moveDK-sound");
  ...
  // Mute Background Button Functionality
  muteBgButton.addEventListener("click", () => {
    isBgMuted = !isBgMuted;
    backgroundSound.muted = isBgMuted;
    muteBgButton.textContent = isBgMuted ? "Unmute Background" : "Mute Background";
  });

  // Mute Move Sounds Button Functionality
  muteMoveButton.addEventListener("click", () => {
    isMoveMuted = !isMoveMuted;
    moveASound.muted = isMoveMuted;
    moveDKSound.muted = isMoveMuted;
    muteMoveButton.textContent = isMoveMuted ? "Unmute Move Sounds" : "Mute Move Sounds";
  });
  ...
  const initialPositions = {
    ...
    'D': [
      [4, 4], [4, 6], [5, 3], [5, 4], [5, 6], [5, 7], [6, 4], [6, 5], [6, 6], [7, 5]
    ],
    'K': [
      [5, 5]
    ]
  ...
  function movePiece(fromRow, fromCol, toRow, toCol) {
    ...
    if (isValidMove(fromRow, fromCol, toRow, toCol)) {
      ...
      // Play move sound based on the piece type if not muted
      if (!isMoveMuted) {
        if (piece === 'A') {
          moveASound.play();
        } else if (piece === 'D' || piece === 'K') {
